...
public class PeerA {
   ...
   */
  public PeerA(InetAddress ip, int tcpDiscussionPort, int tcpPunchPort) throws IOException{
      ...
      public void run() {
        ...
        while (!respRead){
          try{
            ...
            String clientBPublicIP = tokens[3];
            String clientBPublicTcpPort = tokens[4];
            String clientBLocalTcpPort = tokens[5];
            System.out.println("****************************************");
            System.out.println("CLIENT A PUBLIC IP seen by server: " + clientBPublicIP);
            System.out.println("CLIENT A PUBLIC TCP PORT seen by server: " + clientBPublicTcpPort);
            System.out.println("CLIENT A LOCAL  TCP PORT seen by server: " + clientBLocalTcpPort);
            System.out.println("****************************************");
            ...
            proceedHolePunching(
                InetAddress.getByName(clientBPublicIP.trim()),
                Integer.parseInt(clientBLocalTcpPort.trim()),
                Integer.parseInt(clientBPublicTcpPort)
            );
  ...
  public static void main(String[] args) throws IOException{
    ...
    if (args.length > 0) {//Give args
      try {
        ...
        System.out.println("USAGE: java PeerA mediatorIP mediatorTcpDiscussionPort mediatorTcpPunchPort");
        System.out.println("Example: java PeerA 127.0.0.1 9000 9001");
        System.exit(0);
      }
    } else {//Give no args
      System.out.println("PeerA running with default ports 9000 and 9001");
    ...
    new PeerA(mediatorIP, mediatorTcpDiscussionPort, mediatorTcpPunchPort);
  }
}